...
# 将数据中的指定的行进行删除,以及进行：零-均值规范化（z-score标准化）
def nettoyerData(fpath):
    csvFiles = findFileInFiles(fpath, 'allclass.csv')
    for csvFile in csvFiles:
        df = pd.read_csv(fpath + csvFile)
        df = df[~df['248'].isin(['GAMES', 'INTERACTIVE'])]
        Nettoydf = preprocessing.scale(df.ix[:,:-1])
        Nettoydf = pd.DataFrame(Nettoydf)
        Nettoydf['248'] = np.array(df['248'])   # 此处要谨慎，df[248]是带有行号的，要先转化数组的形式
        Nettoydf.to_csv(fpath + csvFile.replace('weka.allclass','newData'),index=False)
        print(csvFile.replace('weka.allclass','newData'),'save ok.')
...
def dealData(fpath):
    ...
    csvFiles = findFileInFiles(fpath, 'newData.csv')
    for csvFile in csvFiles:
        df = pd.read_csv(fpath + csvFile)
...
def getFeatureByGainRatio(fpath):
    ...
    if True:
        ...
        # rankFile = 'wekaTemp.csv'
            ...
            # FeatureGainRatios=np.array(FeatureGainRatios)
            # FeatureGainRatios=FeatureGainRatios[np.isnan(FeatureGainRatios)]=0
            print(FeatureGainRatios)
            # pd.DataFrame(FeatureGainRatios).to_csv(fpath+filter(str.isdigit,rankFile)+'FeatureGainRatios.csv',index=False)    # 将文件进行保存节省调试时间
    ...
    corr = np.array(corr)
    corr = np.nan_to_num(corr)  # 将array中的nan填充为数字0
    print(corr)
    ...
    for weightFeatureGainRatio in normWeightFeatureGainRatios:
        ...
        count = count + 1
    selectFeatureIndex = [] # 用来记录每个set集合的并集。
    mySelect=[]     # 来记录每次set集合中的第一个
    for i in range(len(weightFeatureGainRatioMulCorrs)):    # i表示是第几行的特征值
        if featured[i] in selectFeatureIndex:
            continue
        selectFeatureIndex.append(featured[i])
        mySelect.append(featured[i])
        for j in range(len(weightFeatureGainRatioMulCorrs[i])):
            if featured[j] in selectFeatureIndex:
                continue
            if weightFeatureGainRatioMulCorrs[i][j] >= ((np.max(weightFeatureGainRatioMulCorrs[i]) + np.min(weightFeatureGainRatioMulCorrs[i]))/2*0.5):
                selectFeatureIndex.append(featured[i+j+1])

        if len(selectFeatureIndex)>=200:    # 当集合中剩余：总数-200时保存一次
            pd.DataFrame(mySelect).to_csv(fpath + filter(str.isdigit, rankFile) + 'selectFeatureByRatios200.csv',
                                                    index=False)
        if len(selectFeatureIndex)>=220:    # 当集合中剩余：总数-220时保存一次
            pd.DataFrame(mySelect).to_csv(fpath + filter(str.isdigit, rankFile) + 'selectFeatureByRatios220.csv',
                                                    index=False)

    print(selectFeatureIndex)
    print(mySelect)

    pd.DataFrame(mySelect).to_csv(fpath + filter(str.isdigit, rankFile) + 'selectFeatureByRatios.csv', index=False)
...
def selectFeature(fpath):
    type = 'selectFeatureIndexCustomToday.csv'   # 抽取出来的特征索引顺序表
    ...
    for selectIndex in selectIndexs:
        ...
        if 'Custom' in type:    # 自己定义的索引值从1开始的，所以减一
            selectFeatureIndex = selectFeatureIndex
        ...
        selectFeaturePd['target'] = pd.read_csv(fpath+'entry'+filter(str.isdigit,selectIndex)+'.newData.csv', encoding='utf-8')[
